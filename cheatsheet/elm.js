...
Elm.TryElmx = Elm.TryElmx || {};
Elm.TryElmx.make = function (_elm) {
   "use strict";
   _elm.TryElmx = _elm.TryElmx || {};
   if (_elm.TryElmx.values) return _elm.TryElmx.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var testRows = function (_p0) {    return $Basics.toString(A2(F2(function (x,y) {    return x + y;}),1,$List.length(A2($String.split,"\n",_p0))));};
   var viewTitle = function (test) {
      return A2($Html.div,
      _U.list([A2($Html$Attributes.attribute,"class","test-title")]),
      _U.list([A2($Html.h2,_U.list([A2($Html$Attributes.attribute,"class","title")]),_U.list([$Html.text(A2($String.dropLeft,1,test.title))]))]));
   };
   var CodeCompiled = F2(function (a,b) {    return {ctor: "CodeCompiled",_0: a,_1: b};});
   var Update = F2(function (a,b) {    return {ctor: "Update",_0: a,_1: b};});
   var viewTest = F2(function (address,test) {
      var title = _U.eq(test.title,"") ? A2($Html.span,_U.list([]),_U.list([])) : A2($Html.h3,
      _U.list([A2($Html$Attributes.attribute,"class","title bg-primary")]),
      _U.list([$Html.text(test.title)]));
      var updateMessage = function (_p1) {    return A2($Signal.message,address,A2(Update,test,_p1));};
      var onInput = A3($Html$Events.on,"input",$Html$Events.targetValue,updateMessage);
      return A2($Html.div,
      _U.list([A2($Html$Attributes.attribute,"class","test")]),
      _U.list([title
              ,A2($Html.div,
              _U.list([A2($Html$Attributes.attribute,"class","row")]),
              _U.list([A2($Html.div,
                      _U.list([A2($Html$Attributes.attribute,"class","col-sm-6")]),
                      _U.list([A2($Html.div,
                              _U.list([A2($Html$Attributes.attribute,"class","text-right")]),
                              _U.list([A2($Html.small,_U.list([]),_U.list([$Html.text("elmx")]))]))
                              ,A2($Html.textarea,
                              _U.list([onInput,A2($Html$Attributes.attribute,"rows",testRows(test.code))]),
                              _U.list([$Html.text(test.code)]))]))
                      ,A2($Html.div,
                      _U.list([A2($Html$Attributes.attribute,"class","col-sm-6")]),
                      _U.list([A2($Html.div,
                              _U.list([A2($Html$Attributes.attribute,"class","text-right")]),
                              _U.list([A2($Html.small,_U.list([]),_U.list([$Html.text("elm")]))]))
                              ,A2($Html.textarea,
                              _U.list([A2($Html$Attributes.attribute,"readonly","true"),A2($Html$Attributes.attribute,"rows",testRows(test.elm))]),
                              _U.list([$Html.text(test.elm)]))]))]))]));
   });
   var viewModelItem = F2(function (address,test) {
      return _U.eq(test.code,"") && A2($String.startsWith,"=",test.title) ? viewTitle(test) : A2(viewTest,address,test);
   });
   var view = F2(function (address,model) {
      ...
      _U.list([]),
      _U.list([$Html.text("\r\n        Interactive ")
              ,A2($Html.a,_U.list([A2($Html$Attributes.attribute,"href","https://github.com/pzavolinsky/elmx")]),_U.list([$Html.text("elmx")]))
              ,$Html.text("\r\n        cheatsheet\r\n      ")]))]),
      A2($Basics._op["++"],A2($List.map,viewModelItem(address),model),_U.list([]))));
   });
   var Init = function (a) {    return {ctor: "Init",_0: a};};
   var NoOp = {ctor: "NoOp"};
   var Test = F4(function (a,b,c,d) {    return {id: a,title: b,code: c,elm: d};});
   ...
   var onInit = A2($Signal.map,Init,init);
   var codeCompiled = Elm.Native.Port.make(_elm).inboundSignal("codeCompiled",
   "( String, String )",
   function (v) {
      return typeof v === "object" && v instanceof Array ? {ctor: "_Tuple2"
                                                           ,_0: typeof v[0] === "string" || typeof v[0] === "object" && v[0] instanceof String ? v[0] : _U.badPort("a string",
   ...
   var onCodeCompiled = A2($Signal.map,function (_p2) {    var _p3 = _p2;return A2(CodeCompiled,_p3._0,_p3._1);},codeCompiled);
   var compileCodeMailbox = $Signal.mailbox({ctor: "_Tuple2",_0: "",_1: ""});
   var compileCode = Elm.Native.Port.make(_elm).outboundSignal("compileCode",function (v) {    return [v._0,v._1];},compileCodeMailbox.signal);
   var compileTest = function (test) {
      return $Effects.task(A2($Task.map,
      function (_p4) {
         return NoOp;
      },
      A2($Signal.send,compileCodeMailbox.address,{ctor: "_Tuple2",_0: test.id,_1: test.code})));
   };
   var update = F2(function (action,model) {
      var _p5 = action;
      switch (_p5.ctor)
      {case "Init": var toTest = F2(function (i,_p6) {    var _p7 = _p6;return {id: $Basics.toString(i),title: _p7._0,code: _p7._1,elm: ""};});
           var newModel = A2($List.indexedMap,toTest,_p5._0);
           var eff = $Effects.batch(A2($List.map,compileTest,newModel));
           return {ctor: "_Tuple2",_0: newModel,_1: eff};
         case "Update": var _p8 = _p5._0;
           var newTest = _U.update(_p8,{code: _p5._1});
           var newModel = A2($List.map,function (t) {    return _U.eq(t,_p8) ? newTest : t;},model);
           var eff = compileTest(newTest);
           return {ctor: "_Tuple2",_0: newModel,_1: eff};
         case "CodeCompiled": var newModel = A2($List.map,function (t) {    return _U.eq(t.id,_p5._0) ? _U.update(t,{elm: _p5._1}) : t;},model);
           return {ctor: "_Tuple2",_0: newModel,_1: $Effects.none};
         default: return {ctor: "_Tuple2",_0: model,_1: $Effects.none};}
   });
   var app = $StartApp.start({init: {ctor: "_Tuple2",_0: _U.list([]),_1: $Effects.none},view: view,update: update,inputs: _U.list([onInit,onCodeCompiled])});
   var main = app.html;
   var tasks = Elm.Native.Task.make(_elm).performSignal("tasks",app.tasks);
   return _elm.TryElmx.values = {_op: _op
                                ,app: app
                                ,main: main
                                ,compileCodeMailbox: compileCodeMailbox
                                ,onCodeCompiled: onCodeCompiled
                                ,onInit: onInit
                                ,Test: Test
                                ,NoOp: NoOp
                                ,Init: Init
