...
Scenario: "attr"
  ...
  Then the elm is Html.img [Html.Attributes.attribute "id" "attr"] []
...
Scenario: {:attrs}
  ...
  Then the elm is Html.img (attrs) []

Scenario: "attr" and {:attrs}
  When processing <img id="i1" {:attrs} />
  Then the elm is Html.img ([Html.Attributes.attribute "id" "i1"] ++ attrs) []

Scenario: onClick
  When processing <button onClick={Clicked}>Click me</button>
  Then the elm is Html.button [Html.Events.onClick (Clicked)] [Html.text "Click me"]
...
Scenario: Understands repeated constructs in attribute expressions
  ...
  Then the elm is Html.img [Html.Attributes.attribute "src" ("http://foo.com/bar" ++ authorId ++ ".png")] []
